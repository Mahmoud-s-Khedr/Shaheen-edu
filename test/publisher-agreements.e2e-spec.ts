...
import { AccountStatus, ContentStatus, Role } from '../src/common/types/roles.enum';
...
describe('Publisher agreements and pricing (e2e)', () => {
  ...
  let ids: { courseId: string; chapterId: string; lessonId: string; publisherId: string };

  beforeAll(async () => {
    app = await createTestApp(); await cleanDatabase(app); await flushTestRedis(app);
    ...
    const lesson = await prisma.lesson.create({ data: { chapterId: chapter.id, title: 'Lesson', slug: 'lesson', sortOrder: 1, createdById: actor.id, updatedById: actor.id } });
    ids = { courseId: course.id, chapterId: chapter.id, lessonId: lesson.id, publisherId: publisher.id };
  });
