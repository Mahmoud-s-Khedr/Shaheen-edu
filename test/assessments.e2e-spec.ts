...
describe('Assessments (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    const gradeId = (
      await seedPublishedAcademicGrade(app, 'assessments-e2e-grade')
    ).id;
    const governorate = await prisma.governorate.upsert({
      where: { nameAr: 'محافظة اختبار التقييمات' },
      create: { nameAr: 'محافظة اختبار التقييمات' },
      update: {},
    });
    student1 = await registerStudent(
      app,
      gradeId,
      governorate.id,
      '29902020211111',
    ...
    student2 = await registerStudent(
      app,
      gradeId,
      governorate.id,
      '29902020222222',
    ...
    const course = await prisma.course.create({
      data: {
        subjectId: subject.id,
        title: 'Assessments Course',
        slug: 'assessments-course',
        sortOrder: 1,
        status: ContentStatus.PUBLISHED,
        publishedAt: now,
    ...
    const source = await prisma.questionSource.create({
      data: {
        type: QuestionSourceType.PLATFORM,
        titleAr: 'منصة',
        status: ContentStatus.PUBLISHED,
        publishedAt: now,
        createdById: admin.id,
    ...
    for (let i = 1; i <= 3; i++) {
      const question = await prisma.question.create({
        data: {
          ...
          options: {
            create: [
              { body: 'Correct', isCorrect: true, sortOrder: 1 },
              { body: 'Wrong', isCorrect: false, sortOrder: 2 },
            ],
          },
  ...
  it('keeps a student-generated assessment private to its owner', async () => {
    ...
    expect(
      JSON.parse(ownList.body).data.some(
        (x: any) => x.id === studentAssessmentId,
      ),
    ).toBe(true);
    ...
    expect(
      JSON.parse(otherList.body).data.some(
        (x: any) => x.id === studentAssessmentId,
      ),
    ).toBe(false);
  ...
  it('runs the full attempt lifecycle: start, autosave, submit, result', async () => {
    const start = await app.inject({
      method: 'POST',
      url: `/api/v1/student/assessments/${studentAssessmentId}/attempts/start`,
      headers: { authorization: `Bearer ${student1.accessToken}` },
    });
    ...
    const current = await app.inject({
      method: 'GET',
      url: `/api/v1/student/assessments/${studentAssessmentId}/attempts/current`,
      headers: { authorization: `Bearer ${student1.accessToken}` },
    });
    ...
    expect(
      currentState.questions.find((q: any) => q.id === firstQuestion.id)
        .answered,
    ).toBe(true);
    ...
    const markedCurrent = await app.inject({
      method: 'GET',
      url: `/api/v1/student/assessments/${studentAssessmentId}/attempts/current`,
      headers: { authorization: `Bearer ${student1.accessToken}` },
    });
    ...
    expect(
      JSON.parse(markedCurrent.body).questions.find(
        (q: any) => q.id === firstQuestion.id,
      ).isMarked,
    ).toBe(true);
    ...
    const unmarkedCurrent = await app.inject({
      method: 'GET',
      url: `/api/v1/student/assessments/${studentAssessmentId}/attempts/current`,
      headers: { authorization: `Bearer ${student1.accessToken}` },
    });
    ...
    expect(
      JSON.parse(unmarkedCurrent.body).questions.find(
        (q: any) => q.id === firstQuestion.id,
      ).isMarked,
    ).toBe(false);

    const concurrentActiveTimes = await Promise.all(
      [20, 30].map((activeSeconds) =>
        app.inject({
          method: 'PATCH',
          url: `/api/v1/student/assessments/${studentAssessmentId}/attempts/current/questions/${firstQuestion.id}/active-time`,
          headers: { authorization: `Bearer ${student1.accessToken}` },
    ...
    expect(
      concurrentActiveTimes.map((response) => response.statusCode),
    ).toEqual([200, 200]);
    expect(
      Math.max(
        ...concurrentActiveTimes.map(
          (response) => JSON.parse(response.body).activeSeconds,
        ),
      ),
    ).toBe(30);
    ...
    const submit = await app.inject({
      method: 'POST',
      url: `/api/v1/student/assessments/${studentAssessmentId}/attempts/current/submit`,
      headers: { authorization: `Bearer ${student1.accessToken}` },
    });
    ...
    const result = await app.inject({
      method: 'GET',
      url: `/api/v1/student/assessments/${studentAssessmentId}/attempts/current/result`,
      headers: { authorization: `Bearer ${student1.accessToken}` },
    });
    ...
    expect(resultBody.questions).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ outcome: 'CORRECT' }),
        expect.objectContaining({ outcome: 'OMITTED' }),
      ]),
    );
    expect(
      (
        await prisma.questionCommunityStat.aggregate({
          where: { questionId: { in: questionIds } },
          _sum: { totalResponses: true, correctResponses: true },
        })
      )._sum,
    ).toEqual({ totalResponses: 1, correctResponses: 1 });

    const resultWithoutComparison = await app.inject({
      method: 'GET',
      url: `/api/v1/student/assessments/${studentAssessmentId}/attempts/current/result?includeComparison=false`,
      headers: { authorization: `Bearer ${student1.accessToken}` },
    });
    ...
    const analytics = await app.inject({
      method: 'GET',
      url: '/api/v1/student/assessments/analytics/summary?q=Assessments%20Subject&page=1&limit=1',
      headers: { authorization: `Bearer ${student1.accessToken}` },
    });
    ...
    expect(JSON.parse(analytics.body).meta).toEqual({
      groups: { page: 1, limit: 1, total: 1, totalPages: 1 },
    });
  ...
  it('discovers accessible bank/sources and applies a private mark filter', async () => {
    const banks = await app.inject({
      method: 'GET',
      url: `/api/v1/student/assessments/question-banks?subjectId=${subjectId}`,
      headers: { authorization: `Bearer ${student1.accessToken}` },
    });
    expect(banks.statusCode).toBe(200);
    expect(JSON.parse(banks.body).data).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          id: questionBankId,
    ...
    const sources = await app.inject({
      method: 'GET',
      url: `/api/v1/student/assessments/question-sources?questionBankId=${questionBankId}`,
      headers: { authorization: `Bearer ${student1.accessToken}` },
    });
    expect(sources.statusCode).toBe(200);
    expect(JSON.parse(sources.body).data).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ id: sourceId, type: 'PLATFORM' }),
      ]),
    );
    const mark = await app.inject({
      method: 'POST',
      url: `/api/v1/student/assessments/question-marks/${questionIds[0]}`,
      headers: { authorization: `Bearer ${student1.accessToken}` },
    });
    expect(mark.statusCode).toBe(201);
    const practiceWithMark = await app.inject({
      method: 'GET',
      url: `/api/v1/student/practice/questions?courseId=${courseId}`,
      headers: { authorization: `Bearer ${student1.accessToken}` },
    });
    expect(practiceWithMark.statusCode).toBe(200);
    expect(
      JSON.parse(practiceWithMark.body).data.find(
        (q: any) => q.id === questionIds[0],
      ).isMarked,
    ).toBe(true);
    const marks = await app.inject({
      method: 'GET',
      url: '/api/v1/student/assessments/question-marks',
      headers: { authorization: `Bearer ${student1.accessToken}` },
    });
    expect(marks.statusCode).toBe(200);
    expect(JSON.parse(marks.body).data).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          questionId: questionIds[0],
          bank: expect.objectContaining({ id: questionBankId }),
        }),
      ]),
    );
    ...
    expect(unmark.statusCode).toBe(200);
    const practiceWithoutMark = await app.inject({
      method: 'GET',
      url: `/api/v1/student/practice/questions?courseId=${courseId}`,
      headers: { authorization: `Bearer ${student1.accessToken}` },
    });
    ...
    expect(
      JSON.parse(practiceWithoutMark.body).data.find(
        (q: any) => q.id === questionIds[0],
      ).isMarked,
    ).toBe(false);
  });
    ...
    await Promise.all(
      Array.from({ length: 10 }, (_, index) =>
        prisma.$transaction((tx) =>
          stats.recordResponse(tx, questionId, index < 6),
        ),
      ),
    );
    const aggregate = await prisma.questionCommunityStat.findUniqueOrThrow({
      where: { questionId },
    });
  ...
  it('lets an admin build a custom quiz by hand-picking questions and publish it', async () => {
    ...
    const notYetVisible = await app.inject({
      method: 'GET',
      url: '/api/v1/student/assessments',
      headers: { authorization: `Bearer ${student2.accessToken}` },
    });
    expect(
      JSON.parse(notYetVisible.body).data.some(
    ...
    const publish = await app.inject({
      method: 'POST',
      url: `/api/v1/admin/assessments/${adminAssessmentId}/publish`,
      headers: { authorization: `Bearer ${adminToken}` },
    });
  ...
  it('makes a published admin assessment publicly visible and attemptable by any entitled student', async () => {
    for (const student of [student1, student2]) {
      ...
      const row = JSON.parse(list.body).data.find(
        (x: any) => x.id === adminAssessmentId,
      );
    ...
    const start = await app.inject({
      method: 'POST',
      url: `/api/v1/student/assessments/${adminAssessmentId}/attempts/start`,
      headers: { authorization: `Bearer ${student2.accessToken}` },
    });
    expect(start.statusCode).toBe(201);
    const state = JSON.parse(start.body);
    ...
    const submit = await app.inject({
      method: 'POST',
      url: `/api/v1/student/assessments/${adminAssessmentId}/attempts/current/submit`,
      headers: { authorization: `Bearer ${student2.accessToken}` },
    });
    expect(submit.statusCode).toBe(201);
  });
  ...
  it('archives an admin assessment so it disappears from student lists but keeps existing attempts readable', async () => {
    const archive = await app.inject({
      method: 'POST',
      url: `/api/v1/admin/assessments/${adminAssessmentId}/archive`,
      headers: { authorization: `Bearer ${adminToken}` },
    });
    ...
    expect(
      JSON.parse(list.body).data.some((x: any) => x.id === adminAssessmentId),
    ).toBe(false);
    ...
    const current = await app.inject({
      method: 'GET',
      url: `/api/v1/student/assessments/${adminAssessmentId}/attempts/current`,
      headers: { authorization: `Bearer ${student2.accessToken}` },
    });
    expect(current.statusCode).toBe(200);
    ...
    const result = await app.inject({
      method: 'GET',
      url: `/api/v1/student/assessments/${adminAssessmentId}/attempts/current/result`,
      headers: { authorization: `Bearer ${student2.accessToken}` },
    });
    expect(result.statusCode).toBe(200);
  ...
  it('resumes an in-progress attempt after the assessment is archived, instead of 403ing', async () => {
    const student3 = await registerStudent(
      ...
      (await prisma.governorate.findFirst({
        where: { nameAr: 'محافظة اختبار التقييمات' },
      }))!.id,
      '29902020233333',
      '01033334444',
    ...
    await app.inject({
      method: 'POST',
      url: `/api/v1/admin/assessments/${resumeAssessmentId}/publish`,
      headers: { authorization: `Bearer ${adminToken}` },
    });

    const firstStart = await app.inject({
      method: 'POST',
      url: `/api/v1/student/assessments/${resumeAssessmentId}/attempts/start`,
      headers: { authorization: `Bearer ${student3.accessToken}` },
    });
    ...
    await app.inject({
      method: 'POST',
      url: `/api/v1/admin/assessments/${resumeAssessmentId}/archive`,
      headers: { authorization: `Bearer ${adminToken}` },
    });

    const resumeStart = await app.inject({
      method: 'POST',
      url: `/api/v1/student/assessments/${resumeAssessmentId}/attempts/start`,
      headers: { authorization: `Bearer ${student3.accessToken}` },
    });
  ...
  it('handles two concurrent start requests without erroring, resolving to the same attempt', async () => {
    const student4 = await registerStudent(
      app,
      (await prisma.studentProfile.findUnique({
        where: { userId: student1.userId },
      }))!.academicGradeId!,
    ...
    await app.inject({
      method: 'POST',
      url: `/api/v1/admin/assessments/${raceAssessmentId}/publish`,
      headers: { authorization: `Bearer ${adminToken}` },
    });
    ...
    const [first, second] = await Promise.all([
      app.inject({
        method: 'POST',
        url: `/api/v1/student/assessments/${raceAssessmentId}/attempts/start`,
        headers: { authorization: `Bearer ${student4.accessToken}` },
      }),
      app.inject({
        method: 'POST',
        url: `/api/v1/student/assessments/${raceAssessmentId}/attempts/start`,
    ...
    expect(JSON.parse(first.body).attemptId).toBe(
      JSON.parse(second.body).attemptId,
    );
  });
  ...
  it('only allows deleting a never-published draft assessment', async () => {
    ...
    await app.inject({
      method: 'POST',
      url: `/api/v1/admin/assessments/${publishedId}/publish`,
      headers: { authorization: `Bearer ${adminToken}` },
    });
    const deletePublished = await app.inject({
      method: 'DELETE',
      url: `/api/v1/admin/assessments/${publishedId}`,
      headers: { authorization: `Bearer ${adminToken}` },
    });
  ...
  describe('multi-scope visibility requires entitlement to every scope', () => {
    ...
    beforeAll(async () => {
      const now = new Date();
      const gradeId = (await prisma.studentProfile.findUnique({
        where: { userId: student1.userId },
      }))!.academicGradeId!;
      ...
      const paidCourse = await prisma.course.create({
        data: {
          subjectId: subject.id,
          title: 'Paid Multi-Scope Course',
          slug: 'paid-multi-scope-course',
          sortOrder: 1,
          status: ContentStatus.PUBLISHED,
          publishedAt: now,
      ...
      const create = await app.inject({
        ...
        headers: { authorization: `Bearer ${adminToken}` },
        payload: {
          questionIds: [questionAId, questionBId],
          scopes: [{ chapterId: chapterAId }, { chapterId: chapterBId }],
        },
      });
      expect(create.statusCode).toBe(201);
      ...
      const publish = await app.inject({
        method: 'POST',
        url: `/api/v1/admin/assessments/${leakAssessmentId}/publish`,
        headers: { authorization: `Bearer ${adminToken}` },
      });
      expect(publish.statusCode).toBe(201);
    });
    ...
    it('hides the assessment entirely from a student entitled to only one of its two scopes', async () => {
      ...
      expect(
        JSON.parse(list.body).data.some((x: any) => x.id === leakAssessmentId),
      ).toBe(false);

      const start = await app.inject({
        method: 'POST',
        url: `/api/v1/student/assessments/${leakAssessmentId}/attempts/start`,
        headers: { authorization: `Bearer ${student1.accessToken}` },
      });
      ...
      const direct = await app.inject({
        method: 'GET',
        url: `/api/v1/student/assessments/${leakAssessmentId}`,
        headers: { authorization: `Bearer ${student1.accessToken}` },
      });
  ...
  describe('autosave/submit race safety', () => {
    ...
    beforeAll(async () => {
      student5 = await registerStudent(
        app,
        (await prisma.studentProfile.findUnique({
          where: { userId: student1.userId },
        }))!.academicGradeId!,
    ...
    async function freshStartedAssessment(studentToken: string) {
      ...
      await app.inject({
        method: 'POST',
        url: `/api/v1/admin/assessments/${assessmentId}/publish`,
        headers: { authorization: `Bearer ${adminToken}` },
      });
      const start = await app.inject({
        method: 'POST',
        url: `/api/v1/student/assessments/${assessmentId}/attempts/start`,
    ...
    it('never lets a concurrent autosave report success while missing from the submitted result', async () => {
      for (let i = 0; i < 8; i++) {
        const { assessmentId, questionId, optionId } =
          await freshStartedAssessment(student5.accessToken);
        ...
        const [autosave, submit] = await Promise.all([
          ...
          app.inject({
            method: 'POST',
            url: `/api/v1/student/assessments/${assessmentId}/attempts/current/submit`,
            headers: { authorization: `Bearer ${student5.accessToken}` },
          }),
        ]);
        ...
        const result = await app.inject({
          method: 'GET',
          url: `/api/v1/student/assessments/${assessmentId}/attempts/current/result`,
          headers: { authorization: `Bearer ${student5.accessToken}` },
        });
        expect(result.statusCode).toBe(200);
