...
export const studentLearningJourney: JourneyDefinition = {
  ...
  async run({ clients, context, factory, step }) {
    ...
    await step(
      'Authoring published public learning content and one practice question',
      async () => {
        const grade = await create('/admin/academic-grades', {
          title: factory.localizedTitle('Learning grade'),
          slug: factory.slug('learning-grade'),
        });
        gradeId = grade.id;
        ...
        const course = await create('/admin/courses', {
          title: factory.title('Learning course'),
          slug: factory.slug('learning-course'),
          subjectId: subject.id,
          accessType: 'PUBLIC',
        });
        courseId = course.id;
        ...
        const chapter = await create('/admin/chapters', {
          title: factory.title('Learning chapter'),
          slug: factory.slug('learning-chapter'),
          courseId,
        });
        chapterId = chapter.id;
        context.created.chapters.push(chapterId);
        const lesson = await create('/admin/lessons', {
          title: factory.title('Learning lesson'),
          slug: factory.slug('learning-lesson'),
          chapterId,
        });
        lessonId = lesson.id;
        context.created.lessons.push(lessonId);
        const section = await create('/admin/sections', {
          title: factory.title('Learning section'),
          slug: factory.slug('learning-section'),
          lessonId,
        });
        sectionId = section.id;
        context.created.sections.push(sectionId);
        const content = await create('/admin/content-items', {
          type: 'TEXT',
          title: factory.title('Learning text'),
          textBody: 'Completion is item-level.',
          placement: { sectionId },
        });
        ...
        const first = await create(`/admin/questions/${questionId}/options`, {
          body: 'Correct',
          isCorrect: true,
        });
        correctOptionId = first.options[0].id;
        const second = await create(`/admin/questions/${questionId}/options`, {
          body: 'Wrong',
          isCorrect: false,
        });
        wrongOptionId = second.options.find(
    ...
    await step(
      'Registering the learner and observing item-level completion',
      async () => {
        ...
        const delivery = await student<any>(
          'GET',
          `/student/content-items/${contentItemId}`,
        );
        expectStatus(delivery, 200);
        assert(
          delivery.body.progress?.completed === false,
          'New content must start incomplete',
        );
        const studyState = await student<any>(
          'PUT',
          `/student/content-items/${contentItemId}/study-state`,
          {},
        );
        expectStatus(studyState, 200);
        assert(
          studyState.body.contentItemId === contentItemId,
          'Study state must be recorded for accessible content',
        );
        const resume = await student<any>('GET', '/student/learning/continue');
        expectStatus(resume, 200);
        assert(
          resume.body.data?.contentItem?.id === contentItemId,
          'Continue learning must return the most recently studied item',
        );
        const search = await student<any>(
          'GET',
          `/student/catalog/search?subjectId=${subjectId}&q=Learning`,
        );
        expectStatus(search, 200);
        assert(
          search.body.data?.some((item: any) => item.id === chapterId),
          'Catalog search must find published hierarchy items',
        );
        expectStatus(await student<any>('GET', '/student/my-subjects'), 200);
        expectStatus(
          await student<any>(
            'POST',
            `/student/content-items/${contentItemId}/complete`,
          ),
          201,
        );
        const repeat = await student<any>(
          'POST',
          `/student/content-items/${contentItemId}/complete`,
        );
        expectStatus(repeat, 201);
        const after = await student<any>(
          'GET',
          `/student/content-items/${contentItemId}`,
        );
        expectStatus(after, 200);
        assert(
          after.body.progress?.completed === true,
          'Completed content must be returned by delivery',
        );
      },
    );
    ...
    await step(
      ...
      async () => {
        const questions = await student<any>(
          'GET',
          `/student/practice/questions?courseId=${courseId}`,
        );
        expectStatus(questions, 200);
        assert(
          questions.body.data?.some(
            (question: any) => question.id === questionId,
          ),
          'Course practice must include descendant section questions',
        );
        const wrong = await student<any>(
          'POST',
          `/student/practice/questions/${questionId}/attempts`,
          { optionIds: [wrongOptionId] },
        );
        expectStatus(wrong, 201);
        assert(
          wrong.body.isCorrect === false && wrong.body.attemptNumber === 1,
          'First wrong answer must be retained',
        );
        const correct = await student<any>(
          'POST',
          `/student/practice/questions/${questionId}/attempts`,
          { optionIds: [correctOptionId] },
        );
        expectStatus(correct, 201);
        assert(
          correct.body.isCorrect === true &&
            correct.body.attemptNumber === 2 &&
            correct.body.explanation,
          'Correct retry must return feedback',
        );
        ...
        assert(
          history.body.data?.length === 2 &&
            history.body.data[0].selectedOptionIds[0] === wrongOptionId &&
            history.body.data[1].selectedOptionIds[0] === correctOptionId,
          'Attempt history must preserve each selected answer',
        );
        const missingAsset = await student<any>(
          'GET',
          `/student/practice/questions/${questionId}/assets/missing/access`,
        );
        expectStatus(missingAsset, 404);
        const performance = await student<any>('GET', '/student/performance');
        expectStatus(performance, 200);
        assert(
          performance.body.attemptedQuestions === 1 &&
            performance.body.solvedQuestions === 1 &&
            performance.body.totalAttempts === 2,
          'Performance must summarize the retry history',
        );
      },
    );
    ...
    await step(
      ...
      async () => {
        ...
        const select = await clients.public.request<any>(
          'POST',
          '/auth/parents/select-child',
          { studentUserId: studentId },
          { accessToken: login.body.accessToken },
        );
        expectStatus(select, 201);
        const parent = await clients.public.request<any>(
          'GET',
          '/parent/selected-child/performance',
          undefined,
          { accessToken: select.body.accessToken },
        );
