...
export const manualCommerceJourney: JourneyDefinition = {
  ...
  async run({ clients, context, factory, step }) {
    ...
    await step('Creating an active manual payment method and student', async () => {
      const method = await admin.request<any>('POST', '/admin/manual-payment-methods', { titleAr: methodTitleAr, instructionsAr: 'حوّل المبلغ إلى الحساب المحدد' }); expectStatus(method, 201); methodId = method.body.id;
    ...
    await step('Managing payment methods and a cancellable course order', async () => {
      const listed = await admin.request<any>('GET', `/admin/manual-payment-methods?limit=1&q=${encodeURIComponent(methodSearchTerm)}`); expectStatus(listed, 200); assert(listed.body.data.some((x: any) => x.id === methodId) && listed.body.meta.total >= 1, 'Created payment method must be searchable and paginated for admins');
