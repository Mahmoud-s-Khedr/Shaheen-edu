...
export const manualCommerceJourney: JourneyDefinition = {
  ...
  async run({ clients, context, factory, step }) {
    ...
    await step('Rejecting a receipt and accepting replacement proof', async () => {
      ...
      const replacement = await clients.public.upload<any>(`/student/orders/${orderId}/payment-proof`, { buffer: proofImage, filename: `${filePrefix}-receipt-2.jpg`, contentType: 'image/jpeg' }, { accessToken: studentToken, fields: { transactionReference: 'REF-2' }, headers: { 'idempotency-key': factory.slug('proof') } }); expectStatus(replacement, 201);
