...
export const studentJourney: JourneyDefinition = {
  ...
  async run({ clients, context, factory, step }) {
    const phone = factory.phone(); const parentPhone = factory.phone(); const nationalId = factory.nationalId(); const password = factory.password('Student'); let academicGradeId = ''; let governorateId = '';
    await step('Fetching a published grade for student registration', async () => {
      const governorate = await clients.admin.request<any>('POST', '/admin/geography/governorates', factory.localizedTitle('Registration governorate'));
      expectStatus(governorate, 201); governorateId = governorate.body.id;
    ...
    await step('Reading and safely updating student profile', async () => {
      ...
      const update = await clients.student.request<any>('PATCH', '/students/me', { centerId: null }); expectStatus(update, 200); assert(update.body.studentProfile?.center === null, 'Allowed profile update must persist');
