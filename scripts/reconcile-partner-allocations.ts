...
async function main() {
  ...
  const [allocations, approvedItems] = await Promise.all([
    prisma.partnerAllocation.findMany({ where: Object.keys(dateRange).length ? { createdAt: dateRange } : {}, include: { orderItem: { include: { order: { select: { id: true, status: true, approvedAt: true } } } } } }),
    prisma.orderItem.findMany({ where: { order: { status: OrderStatus.APPROVED, ...(Object.keys(dateRange).length ? { approvedAt: dateRange } : {}) } }, include: { allocations: { select: { id: true, kind: true, amountMinor: true } } } }),
  ]);
  const issues = allocations.flatMap((allocation) => {
    ...
    if (allocation.basisMinor !== item.priceMinor) reasons.push('basis_does_not_match_final_item_price');
    if (allocation.amountMinor <= 0 || allocation.amountMinor > item.priceMinor) reasons.push('invalid_allocation_amount');
  ...
  console.log(JSON.stringify({ mode: 'read-only', period: { from: from ?? null, to: to ?? null }, allocationsScanned: allocations.length, approvedOrderItemsScanned: approvedItems.length, issues: { count: issues.length, sample: issues.slice(0, 100) }, coverage: { approvedItemsWithoutPartnerAllocation: unallocatedApprovedItems.length, sample: unallocatedApprovedItems.slice(0, 100), note: 'These are candidates for review, not automatic errors: an item may have no publisher agreement and no referral.' } }, null, 2));
}
