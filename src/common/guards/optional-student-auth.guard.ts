...
export class OptionalStudentAuthGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    try {
      const payload = jwt.verify(header.slice(7), this.config.get('jwt', { infer: true }).accessSecret) as { typ: string; sub: string; sid: string };
      if (payload.typ !== 'user_access') throw new Error();
      const [session, user] = await Promise.all([this.prisma.authSession.findUnique({ where: { id: payload.sid } }), this.prisma.user.findUnique({ where: { id: payload.sub } })]);
      if (!session || session.revoked || session.expiresAt < new Date() || !user || user.status !== AccountStatus.ACTIVE) throw new Error();
