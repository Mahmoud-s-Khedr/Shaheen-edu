...
@ApiTags('student/assessments') @ApiBearerAuth() @UseGuards(RolesGuard) @Roles(Role.STUDENT)
...
export class AssessmentsController {
  ...
  @Patch(':id') @ApiOperation({ summary: 'Rename an owned assessment' }) @ApiOkResponse({ type: AssessmentDetailDto }) @ApiStandardErrors(400, 401, 403, 404)
  rename(@CurrentUser() user: RequestUser, @Param('id') id: string, @Body() dto: RenameAssessmentDto) { return this.assessments.rename(user.id, id, dto); }

  @Delete(':id') @ApiOperation({ summary: 'Delete an owned assessment' }) @ApiOkResponse({ type: IdDeletedResponseDto }) @ApiStandardErrors(401, 403, 404)
  remove(@CurrentUser() user: RequestUser, @Param('id') id: string) { return this.assessments.remove(user.id, id); }

  @Post(':id/attempts/start') @ApiOperation({ summary: 'Start or resume the attempt for an assessment' }) @ApiCreatedResponse({ type: AssessmentAttemptStateDto }) @ApiStandardErrors(401, 403, 404, 409)
  start(@CurrentUser() user: RequestUser, @Param('id') id: string) { return this.assessments.startAttempt(user.id, id); }

  @Get(':id/attempts/current') @ApiOperation({ summary: 'Get the current (resumable) attempt state' }) @ApiOkResponse({ type: AssessmentAttemptStateDto }) @ApiStandardErrors(401, 403, 404)
  current(@CurrentUser() user: RequestUser, @Param('id') id: string) { return this.assessments.currentAttemptState(user.id, id); }
...
export class AdminAssessmentsController {
  ...
  @Post('standard') @ApiOperation({ summary: 'Create a quiz/exam via random sample from the question bank' }) @ApiCreatedResponse({ type: AdminAssessmentListItemDto }) @ApiStandardErrors(400, 401, 403)
  createStandard(@CurrentUser() user: RequestUser, @Body() dto: GenerateStandardAssessmentDto) { return this.assessments.createStandard(user, dto); }

  @Post('custom') @ApiOperation({ summary: 'Create a quiz/exam by hand-picking questions from the question bank' }) @ApiCreatedResponse({ type: AdminAssessmentListItemDto }) @ApiStandardErrors(400, 401, 403)
  createCustom(@CurrentUser() user: RequestUser, @Body() dto: CreateCustomAssessmentDto) { return this.assessments.createCustom(user, dto); }

  @Get() @ApiOperation({ summary: 'List admin-created assessments' }) @ApiOkResponse({ type: PaginatedAdminAssessmentsResponseDto }) @ApiStandardErrors(401, 403)
  list(@CurrentUser() user: RequestUser, @Query() query: QueryAdminAssessmentDto) { return this.assessments.listAdmin(user, query); }

  @Get(':id') @ApiOperation({ summary: 'Get an admin-created assessment, including correct answers' }) @ApiOkResponse({ type: AdminAssessmentListItemDto }) @ApiStandardErrors(401, 403, 404)
  get(@CurrentUser() user: RequestUser, @Param('id') id: string) { return this.assessments.getAdmin(user, id); }

  @Patch(':id') @ApiOperation({ summary: 'Update a draft assessment (title, mode, timer)' }) @ApiOkResponse({ type: AdminAssessmentListItemDto }) @ApiStandardErrors(400, 401, 403, 404, 409)
  update(@CurrentUser() user: RequestUser, @Param('id') id: string, @Body() dto: UpdateAdminAssessmentDto) { return this.assessments.updateAdmin(user, id, dto); }

  @Post(':id/publish') @ApiOperation({ summary: 'Publish a draft assessment' }) @ApiCreatedResponse({ type: AdminAssessmentListItemDto }) @ApiStandardErrors(401, 403, 404, 409)
  publish(@CurrentUser() user: RequestUser, @Param('id') id: string) { return this.assessments.publish(user, id); }

  @Post(':id/archive') @ApiOperation({ summary: 'Archive a published assessment' }) @ApiCreatedResponse({ type: AdminAssessmentListItemDto }) @ApiStandardErrors(401, 403, 404, 409)
  archive(@CurrentUser() user: RequestUser, @Param('id') id: string) { return this.assessments.archive(user, id); }

  @Delete(':id') @ApiOperation({ summary: 'Delete a never-published draft assessment' }) @ApiOkResponse({ type: IdDeletedResponseDto }) @ApiStandardErrors(401, 403, 404, 409)
  remove(@CurrentUser() user: RequestUser, @Param('id') id: string) { return this.assessments.deleteAdmin(user, id); }
}
