...
describe('PublisherAgreementsService replacement', () => {
  it('ends the predecessor and creates an active successor version atomically', async () => {
    ...
    const tx: any = {
      publisherAgreement: {
        update: jest.fn(),
        create: jest.fn().mockResolvedValue({ id: 'agreement-2' }),
      },
    };
    const prisma: any = {
      ...
      partnerProfile: {
        findUnique: jest
          .fn()
          .mockResolvedValue({ partnerType: 'CONTENT_PUBLISHER' }),
      },
      course: { findUnique: jest.fn().mockResolvedValue({ id: 'course-1' }) },
    };
    const service = new PublisherAgreementsService(prisma, {
      record: jest.fn(),
    } as any);
    jest
      .spyOn(service, 'getOrThrow')
    ...
    expect(tx.publisherAgreement.update).toHaveBeenCalledWith({
    ...
    expect(tx.publisherAgreement.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({
          supersedesId: 'agreement-1',
          version: 3,
          status: PublisherAgreementStatus.ACTIVE,
        }),
      }),
