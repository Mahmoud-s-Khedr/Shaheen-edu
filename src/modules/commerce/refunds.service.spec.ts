...
describe('RefundsService', () => {
  ...
  function build(
    policy = { eligibilityWindowDays: 7, maximumConsumptionBps: 1_000 },
  ) {
    const tx: any = {
      ...
      refundPolicy: {
        findFirst: jest
          .fn()
          .mockResolvedValue({ id: 'policy-1', version: 1, ...policy }),
      },
    };
    const prisma: any = {
  ...
  it('keeps an eligible request pending for manual review', async () => {
    ...
    tx.refundRequest.create.mockImplementation(async ({ data }: any) => ({
      id: 'refund-1',
      ...data,
      items: data.items.create.map((item: any) => ({
        ...item,
        id: 'refund-item-1',
        orderItem: approvedOrder.items[0],
      })),
    }));

    const result = await service.request(studentUserId, approvedOrder.id, {
    ...
    expect(tx.refundRequest.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ status: RefundRequestStatus.PENDING }),
      }),
    );
  });
  ...
  it('approval revokes the purchased entitlement and appends a payable negative allocation', async () => {
    ...
    tx.refundRequest.findUniqueOrThrow.mockResolvedValue({
      ...request,
      status: RefundRequestStatus.APPROVED,
      manualRefundReference: 'WHATSAPP-REF-1',
      items: [
        {
          id: 'refund-item-1',
          orderItemId: 'item-1',
    ...
    expect(tx.studentEntitlement.updateMany).toHaveBeenCalledWith(
      expect.objectContaining({
        where: expect.objectContaining({ orderItemId: { in: ['item-1'] } }),
      }),
    );
    expect(tx.partnerAllocation.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({
          amountMinor: -2400,
          basisMinor: -12000,
          state: 'PAYABLE',
          reversedAllocationId: 'allocation-1',
        }),
      }),
    );
    expect(tx.partnerAllocation.updateMany).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ state: 'REVERSED' }),
      }),
    );
  });
});
