...
import { AssetKind, AssetStatus, CommerceTargetType, ManualPaymentSubmissionStatus, OrderStatus } from '../../../common/types/roles.enum';
...
export class UpdatePaymentMethodDto {
  @ApiPropertyOptional({ example: 'فودافون كاش' }) @IsOptional() @IsString() @MinLength(1) @MaxLength(120) titleAr?: string;
  @ApiPropertyOptional({ example: 'حوّل المبلغ ثم ارفع الإيصال.' }) @IsOptional() @IsString() @MinLength(1) @MaxLength(4000) instructionsAr?: string;
  @ApiPropertyOptional({ example: 'Vodafone Cash', nullable: true }) @IsOptional() @IsString() @MaxLength(120) titleEn?: string | null;
  @ApiPropertyOptional({ example: 'Transfer the amount, then upload the receipt.', nullable: true }) @IsOptional() @IsString() @MaxLength(4000) instructionsEn?: string | null;
  @ApiPropertyOptional({ example: true }) @IsOptional() @IsBoolean() isActive?: boolean;
}
export class ReorderPaymentMethodsDto { @ApiProperty({ type: [String], example: ['payment-method-id-1', 'payment-method-id-2'] }) @IsString({ each: true }) methodIds!: string[]; }
...
export class ManualPaymentMethodDto {
  ...
  @ApiPropertyOptional({ type: String, format: 'date-time', example: '2026-08-04T10:00:00.000Z' }) createdAt?: Date;
  @ApiPropertyOptional({ type: String, format: 'date-time', example: '2026-08-04T10:00:00.000Z' }) updatedAt?: Date;
}
...
export class OrderDto {
  ...
  @ApiProperty({ type: MoneyDto }) total!: MoneyDto;
  @ApiProperty({ type: PaymentMethodSnapshotDto }) paymentMethod!: PaymentMethodSnapshotDto;
  @ApiProperty({ type: String, format: 'date-time', example: '2026-08-04T10:00:00.000Z' }) createdAt!: Date;
  @ApiProperty({ type: String, format: 'date-time', example: '2026-08-04T10:30:00.000Z', nullable: true }) approvedAt!: Date | null;
  @ApiProperty({ type: String, format: 'date-time', example: '2026-08-04T10:15:00.000Z', nullable: true }) cancelledAt!: Date | null;
  @ApiProperty({ type: [OrderItemDto] }) items!: OrderItemDto[];
  @ApiProperty({ type: [PaymentSubmissionSummaryDto] }) submissions!: PaymentSubmissionSummaryDto[];
}
...
export class IdDeletedResponseDto { @ApiProperty({ example: 'cart-item-id' }) id!: string; @ApiProperty({ example: true }) deleted!: boolean; }
export class IdStatusResponseDto { @ApiProperty({ example: 'payment-submission-id' }) id!: string; @ApiProperty({ enum: ManualPaymentSubmissionStatus }) status!: ManualPaymentSubmissionStatus; }
...
export class PaginatedPaymentSubmissionsResponseDto {
  @ApiProperty({ type: [PaymentSubmissionAdminDto] }) data!: PaymentSubmissionAdminDto[];
  @ApiProperty({ example: { page: 1, limit: 20, total: 1, totalPages: 1 } }) meta!: { page: number; limit: number; total: number; totalPages: number };
}
...
export class ProtectedProofDto {
  ...
  @ApiProperty({ example: 'https://storage.example.test/protected/receipt.png' }) url!: string;
  @ApiProperty({ type: String, format: 'date-time', example: '2026-08-04T10:10:00.000Z' }) expiresAt!: Date;
}
...
export class PaymentProofAssetDto {
  ...
  @ApiProperty({ enum: AssetKind, example: AssetKind.PAYMENT_PROOF }) kind!: AssetKind;
  @ApiProperty({ enum: AssetStatus, example: AssetStatus.UPLOADING }) status!: AssetStatus;
  @ApiProperty({ example: 'receipt.png' }) filename!: string;
  @ApiProperty({ example: 'image/png' }) mimeType!: string;
  @ApiProperty({ example: null, nullable: true }) sizeBytes!: number | null;
  @ApiProperty({ example: null, nullable: true }) checksum!: string | null;
  @ApiProperty({ type: String, format: 'date-time', example: '2026-08-04T10:00:00.000Z' }) createdAt!: Date;
  @ApiProperty({ type: String, format: 'date-time', example: null, nullable: true }) readyAt!: Date | null;
  @ApiProperty({ type: String, format: 'date-time', example: null, nullable: true }) failedAt!: Date | null;
  @ApiProperty({ type: String, format: 'date-time', example: null, nullable: true }) archivedAt!: Date | null;
}

export class UploadInstructionDto {
  @ApiProperty({ example: 'https://storage.example.test/upload/signed-url' }) url!: string;
  @ApiProperty({ example: 'PUT' }) method!: string;
  @ApiProperty({ example: { 'content-type': 'image/png' } }) headers!: Record<string, string>;
  @ApiProperty({ type: String, format: 'date-time', example: '2026-08-04T10:15:00.000Z' }) expiresAt!: Date;
}
