...
export class RefundsService {
  ...
  async request(studentUserId: string, orderId: string, dto: CreateRefundRequestDto) {
    const now = new Date();
    try {
      const refund = await this.prisma.$transaction(async (tx) => {
        ...
        const requestedIds = dto.orderItemIds ? [...new Set(dto.orderItemIds)] : order.items.map((item: any) => item.id);
        if (dto.orderItemIds && requestedIds.length !== dto.orderItemIds.length) throw new BadRequestException('Each order item can be requested only once');
        ...
        return tx.refundRequest.create({
          data: {
            ...
            rejectionReason: automaticallyRejected ? rejectionReasons.join(', ') : null,
            reviewedAt: automaticallyRejected ? now : null,
            items: { create: items.map((item: any) => ({ orderItemId: item.id, amountMinor: item.priceMinor, currency: item.currency })) },
          },
          include: { items: { include: { orderItem: true } } },
        });
      }, { isolationLevel: 'Serializable' });
      await this.audit.record({ actorUserId: studentUserId, action: refund.status === RefundRequestStatus.REJECTED ? 'REFUND_REQUEST_AUTO_REJECTED' : 'REFUND_REQUESTED', targetType: 'RefundRequest', targetId: refund.id, metadata: { orderId, orderItemIds: refund.items.map((item: any) => item.orderItemId) } });
      return this.response(refund);
    } catch (error: any) {
      if (error?.code === 'P2002') throw new ConflictException('A selected order item already has a refund request');
      throw error;
    }
  }

  async own(studentUserId: string, query: RefundRequestsQueryDto) {
    const where = { studentUserId, ...(query.status ? { status: query.status } : {}) };
    const [data, total] = await this.prisma.$transaction([
      this.prisma.refundRequest.findMany({ where, include: { items: { include: { orderItem: true } } }, orderBy: [{ requestedAt: 'desc' }, { id: 'desc' }], skip: (query.page - 1) * query.limit, take: query.limit }),
  ...
  async list(actor: RequestUser, query: AdminRefundRequestsQueryDto) {
    this.admin(actor);
    const where = { ...(query.studentUserId ? { studentUserId: query.studentUserId } : {}), ...(query.status ? { status: query.status } : {}) };
    const [data, total] = await this.prisma.$transaction([
      this.prisma.refundRequest.findMany({ where, include: { student: { select: { fullName: true } }, reviewedBy: { select: { loginIdentifier: true } }, items: { include: { orderItem: true } } }, orderBy: [{ requestedAt: 'desc' }, { id: 'desc' }], skip: (query.page - 1) * query.limit, take: query.limit }),
      this.prisma.refundRequest.count({ where }),
    ]);
    return { data: data.map((row) => this.response(row)), meta: toPaginationMeta(query.page, query.limit, total) };
  }
  ...
  async approve(actor: RequestUser, refundId: string, dto: ApproveRefundDto) {
    ...
    const refund = await this.prisma.$transaction(async (tx) => {
      const request = await tx.refundRequest.findUnique({
        ...
        include: {
          items: {
            include: {
              orderItem: {
                include: { allocations: { where: { reversedAllocationId: null } } },
              },
            },
          },
        },
      ...
      const claimed = await tx.refundRequest.updateMany({
        ...
        data: { status: RefundRequestStatus.APPROVED, reviewedById: actor.id, reviewedAt: now, reviewNote: dto.reviewNote?.trim() || null, manualRefundReference: dto.manualRefundReference.trim() },
      });
      if (!claimed.count) throw new ConflictException('Only pending refund requests can be approved');
      const orderItemIds = request.items.map((item: any) => item.orderItemId);
      await tx.studentEntitlement.updateMany({
        where: { studentUserId: request.studentUserId, orderItemId: { in: orderItemIds }, status: EntitlementStatus.ACTIVE },
        data: { status: EntitlementStatus.REVOKED, revokedAt: now, revokedById: actor.id },
      });
      for (const refundItem of request.items) {
        for (const allocation of refundItem.orderItem.allocations.filter((row: any) => row.state !== PartnerAllocationState.REVERSED)) {
          await tx.partnerAllocation.create({
            data: {
              ...
              snapshot: { reversalOfAllocationId: allocation.id, refundRequestId: request.id, originalSnapshot: allocation.snapshot },
              idempotencyKey: `refund-reversal:${request.id}:${allocation.id}`,
              reversedAllocationId: allocation.id,
            },
          });
          const reversed = await tx.partnerAllocation.updateMany({ where: { id: allocation.id, reversedAllocationId: null }, data: { state: PartnerAllocationState.REVERSED, reversedAt: now } });
          if (reversed.count !== 1) throw new ConflictException('An allocation was reversed concurrently');
        }
      }
      return tx.refundRequest.findUniqueOrThrow({ where: { id: request.id }, include: { items: { include: { orderItem: true } }, reviewedBy: { select: { loginIdentifier: true } } } });
    }, { isolationLevel: 'Serializable' });
    await this.audit.record({ actorUserId: actor.id, action: 'REFUND_APPROVED', targetType: 'RefundRequest', targetId: refundId, metadata: { orderId: refund.orderId, manualRefundReference: refund.manualRefundReference, orderItemIds: refund.items.map((item: any) => item.orderItemId) } });
    return this.response(refund);
  }
  ...
  async reject(actor: RequestUser, refundId: string, dto: RejectRefundDto) {
    ...
    const updated = await this.prisma.refundRequest.updateMany({
      where: { id: refundId, status: RefundRequestStatus.PENDING },
      data: { status: RefundRequestStatus.REJECTED, reviewedById: actor.id, reviewedAt: now, rejectionReason: dto.rejectionReason.trim(), reviewNote: dto.reviewNote?.trim() || null },
    });
    if (!updated.count) {
      const exists = await this.prisma.refundRequest.findUnique({ where: { id: refundId }, select: { id: true } });
      if (!exists) throw new NotFoundException('Refund request not found');
      throw new ConflictException('Only pending refund requests can be rejected');
    }
    await this.audit.record({ actorUserId: actor.id, action: 'REFUND_REJECTED', targetType: 'RefundRequest', targetId: refundId, metadata: { rejectionReason: dto.rejectionReason.trim() } });
    return this.prisma.refundRequest.findUniqueOrThrow({ where: { id: refundId }, include: { items: { include: { orderItem: true } }, reviewedBy: { select: { loginIdentifier: true } } } }).then((row) => this.response(row));
  }
}
