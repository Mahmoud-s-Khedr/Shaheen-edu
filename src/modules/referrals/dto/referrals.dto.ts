...
export class UpdateReferralProgramDto {
  ...
  @ApiPropertyOptional() @IsOptional() @IsInt() @Min(1) usageLimit?:
    number | null;
  @ApiPropertyOptional() @IsOptional() @IsInt() @Min(1) perStudentUsageLimit?:
    number | null;
}
...
export class CreateReferralCommissionRuleDto {
  @ApiProperty({ enum: ReferralCommissionKind })
...
export class CreateReferralReviewRuleDto {
  @ApiProperty() @IsString() @MaxLength(160) name!: string;
  @ApiProperty({ enum: ReferralReviewRuleKind })
  @IsEnum(ReferralReviewRuleKind)
  kind!: ReferralReviewRuleKind;
  @ApiProperty({ enum: ReferralReviewAction })
  @IsEnum(ReferralReviewAction)
...
export class UpdateReferralReviewRuleDto {
  ...
  @MaxLength(160)
  name?: string;
  @ApiPropertyOptional({ enum: ReferralReviewAction })
  @IsOptional()
  @IsEnum(ReferralReviewAction)
  action?: ReferralReviewAction;
  @ApiPropertyOptional({ minimum: 1 })
...
export class ReferralReviewFlagsQueryDto extends PaginationQueryDto {
  @ApiPropertyOptional({ enum: ReferralReviewStatus })
  @IsOptional()
  @IsEnum(ReferralReviewStatus)
...
export class AdminReferralReportingQueryDto extends ReferralReportingQueryDto {
  @ApiProperty({ description: 'Referral-partner user ID to report on.' })
  @IsString()
