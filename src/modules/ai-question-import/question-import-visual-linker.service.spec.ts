...
describe('QuestionImportVisualLinkerService', () => {
  ...
  it('blocks possibly clipped and incomplete crops even after approval', () => {
    const requirement = { kind: QuestionImportVisualRequirementKind.QUESTION_FIGURE, owner: 'QUESTION', ownerReference: 'QUESTION', expectedCardinality: 1, sourcePage: 3 };
    const assignment = { mediaKey: 'M1', owner: 'QUESTION', ownerReference: 'QUESTION', status: QuestionImportMediaAssignmentStatus.APPROVED, media: media('M1', QuestionImportMediaCropCompleteness.POSSIBLY_CLIPPED) };
    expect(linker.resolve(requirement, [assignment], [assignment.media])).toMatchObject({ state: QuestionImportVisualResolutionState.INCOMPLETE_CROP });
  });

  it('ranks same-page candidates deterministically', () => {
    const requirement = { kind: QuestionImportVisualRequirementKind.QUESTION_FIGURE, sourcePage: 3, expectedCardinality: 1, sourceEnvelope: { bottom: 250 } };
    const rankings = linker.rank(requirement, [media('M2'), media('M1'), { ...media('M3'), pageNumber: 4 }]);
  ...
  it('does not resolve a figure assigned far from its question geometry', () => {
    ...
    const assignment = {
      ...
      media: { ...media('M1'), normalizedBounds: { left: 50, top: 100, right: 300, bottom: 250 } },
    };
    expect(linker.resolve(requirement, [assignment], [assignment.media])).toMatchObject({
