...
export interface PdfTranscribedPage { content: string; confidence: number; uncertainSpans: string[]; warnings: string[]; }
const pageSchema = {
  name: 'visual_arabic_exam_page_transcription_v1', strict: true,
  schema: { type: 'object', additionalProperties: false, required: ['content', 'confidence', 'uncertainSpans', 'warnings'], properties: {
...
export class PdfTranscriptionClient {
  ...
  async transcribeImage(image: Buffer): Promise<{ page: PdfTranscribedPage; raw: unknown; usage: unknown }> {
    return this.request(image, `You are transcribing a high-resolution image of a visual Arabic exam document page. Preserve the RTL visual reading order, exact wording, punctuation, headings, page numbers, question numbering, and option layout (including columns). Transcribe all meaningful document text, including cover, index, instructions, answer forms, headers, and footers when they contain text. Never invent, normalize, solve, or silently repair text. For every unreadable fragment write [غير مقروء] in content and include that exact placeholder or unclear fragment in uncertainSpans.`);
  }

  async verifyImage(image: Buffer, first: PdfTranscribedPage): Promise<{ page: PdfTranscribedPage; raw: unknown; usage: unknown }> {
    return this.request(image, `You are the independent verification pass for a visual Arabic exam document OCR result. Compare the image against the first attempt below, correct only discrepancies, and return the complete revised transcription. Preserve all meaningful RTL content and layout, including headings, page numbers, question numbering, options, answer forms, headers, and footers. Never guess: render unreadable text as [غير مقروء] and list each unclear fragment in uncertainSpans.\n\nFIRST ATTEMPT:\n${first.content}`);
  }
  ...
  private async request(image: Buffer, instruction: string): Promise<{ page: PdfTranscribedPage; raw: unknown; usage: unknown }> {
    ...
    try {
      ...
      if (!page || typeof page.content !== 'string' || !Array.isArray(page.uncertainSpans) || !Array.isArray(page.warnings)) throw new OpenRouterQuestionImportError('OpenRouter returned invalid image transcription', raw, raw?.usage ?? null);
