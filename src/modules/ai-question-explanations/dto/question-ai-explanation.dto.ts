...
import { ArrayMaxSize, ArrayMinSize, IsArray, IsBoolean, IsEnum, IsInt, IsNotEmpty, IsObject, IsOptional, IsString, MaxLength, Min } from 'class-validator';
import { QuestionAiExplanationRunMode } from '../../../common/types/roles.enum';

export class AiQuestionAnswerDto {
  @ApiPropertyOptional({ type: [Number] })
  @IsOptional() @IsArray() @ArrayMinSize(1) @ArrayMaxSize(20) @Type(() => Number) @IsInt({ each: true }) @Min(0, { each: true })
  selectedOptionIndexes?: number[];

  @ApiPropertyOptional({ type: [String] })
  @IsOptional() @IsArray() @ArrayMinSize(1) @ArrayMaxSize(100) @IsString({ each: true }) @MaxLength(10000, { each: true })
  acceptedAnswers?: string[];

  @ApiPropertyOptional()
  @IsOptional() @IsString() @IsNotEmpty() @MaxLength(100000)
