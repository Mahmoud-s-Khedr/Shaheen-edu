...
import { AccountStatus, EntitlementSource, EntitlementStatus, Role } from '../../common/types/roles.enum';
...
export class EntitlementsService {
  ...
  async grant(actor: RequestUser, dto: GrantEntitlementDto) {
    ...
    if (dto.source === EntitlementSource.PAYMENT)
      throw new BadRequestException('PAYMENT entitlements can only be created by payment approval');
    const student = await this.prisma.user.findFirst({ where: { id: dto.studentUserId, role: Role.STUDENT, status: AccountStatus.ACTIVE } });
    ...
    const entitlement = await this.prisma.$transaction(async (tx) => {
      ...
      return tx.studentEntitlement.create({ data: { studentUserId: dto.studentUserId, courseId: dto.courseId, chapterId: dto.chapterId, source: dto.source ?? EntitlementSource.ADMIN, startsAt: dto.startsAt, expiresAt: dto.expiresAt, grantedById: actor.id } });
    });
