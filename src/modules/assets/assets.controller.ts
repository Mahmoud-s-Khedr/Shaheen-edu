...
export class AssetsController {
  ...
  @Post('upload') @ApiConsumes('multipart/form-data') @ApiOperation({ summary: 'Authorize a direct Bunny file upload' })
  async authorize(@CurrentUser() actor: RequestUser, @Query('kind') kind: AssetKind, @Req() req: any) {
    if (!Object.values(AssetKind).includes(kind)) throw new BadRequestException('Unsupported asset kind');
    const part = await req.file();
    if (!part) throw new BadRequestException('A file is required');
    try {
      return await this.assets.authorizeUpload(actor, kind, { filename: part.filename, mimeType: part.mimetype });
    } finally {
