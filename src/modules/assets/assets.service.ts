...
const documentMimes = new Set(['application/pdf', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', 'application/vnd.openxmlformats-officedocument.presentationml.presentation']);
...
export class AssetsService {
  ...
  private assertMagic(kind: AssetKind, mime: string, first: Buffer) {
    ...
    if ((mime === 'image/png' && !png) || (mime === 'image/jpeg' && !jpg) || (mime === 'image/webp' && !webp) || ((kind === AssetKind.PDF || mime === 'application/pdf') && !pdf)) throw new BadRequestException('File signature does not match declared MIME type');
  }
  ...
  async setCover(actor: RequestUser, resource: string, id: string, assetId: string) {
    ...
    const client = clients[resource]; if (!client) throw new BadRequestException('Unsupported cover resource'); if (!await client.findUnique({ where: { id } })) throw new NotFoundException('Hierarchy record not found');
    await client.update({ where: { id }, data: { coverAssetId: assetId } }); await this.audit.record({ actorUserId: actor.id, action: 'HIERARCHY_COVER_SET', targetType: resource, targetId: id, metadata: { assetId } }); return this.get(actor, assetId);
  }
