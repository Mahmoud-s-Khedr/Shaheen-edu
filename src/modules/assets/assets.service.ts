...
export class AssetsService {
  ...
  async isReferenced(id: string) { const [content, refs, grades, subjects, courses, chapters, lessons, sections, questionAssets, questionVideos, assessmentQuestionVideos, assessmentQuestionAttachments] = await this.prisma.$transaction([this.prisma.contentItem.count({ where: { primaryAssetId: id } }), this.prisma.assetReference.count({ where: { assetId: id } }), this.prisma.academicGrade.count({ where: { coverAssetId: id } }), this.prisma.subject.count({ where: { coverAssetId: id } }), this.prisma.course.count({ where: { coverAssetId: id } }), this.prisma.chapter.count({ where: { coverAssetId: id } }), this.prisma.lesson.count({ where: { coverAssetId: id } }), this.prisma.section.count({ where: { coverAssetId: id } }), this.prisma.questionAsset.count({ where: { assetId: id } }), this.prisma.questionVideoLink.count({ where: { videoAssetId: id } }), this.prisma.assessmentQuestion.count({ where: { videoAssetId: id } }), this.prisma.assessmentQuestionAsset.count({ where: { assetId: id } })]); const paymentProofs = (this.prisma as any).manualPaymentSubmission ? await (this.prisma as any).manualPaymentSubmission.count({ where: { proofAssetId: id } }) : 0; return content + refs + grades + subjects + courses + chapters + lessons + sections + questionAssets + questionVideos + assessmentQuestionVideos + assessmentQuestionAttachments + paymentProofs > 0; }
