...
| Cart, checkout, and payments | `Cart`, `CartItem` — student shopping cart; `ManualPaymentMethod` — staff-managed payment instruction; `Order`, `OrderItem` — checkout snapshot; `PaymentAttempt` — manual/XPay attempt; `PaymentReceipt` — payment receipt; `ManualPaymentSubmission` — uploaded proof and staff decision; `XPayWebhookEvent` — retained provider callback; `CommerceIdempotencyKey` — request replay protection. |
...
| `/student/orders/*`, XPay attempt/webhook, payment-proof upload/resubmit | `Order`, `OrderItem`, `PaymentAttempt`, `ManualPaymentSubmission`, `ManualPaymentMethod`, `Asset`, `XPayWebhookEvent` | `Order`, `PaymentAttempt`, `ManualPaymentSubmission`, payment-proof `Asset`, `XPayWebhookEvent`, `StudentEntitlement`, `PartnerAllocation` |
...
- Bunny and XPay callbacks persist `BunnyStreamWebhookEvent` / `XPayWebhookEvent`
