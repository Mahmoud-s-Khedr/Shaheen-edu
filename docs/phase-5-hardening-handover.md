...
| Attribution and fulfilment | Approved manual-payment and XPay paths retain checkout attribution, create at most one publisher allocation and one referral allocation per eligible order item, and retain the same result after duplicate approval/webhook delivery.                                                                                                                     |
...
5. Exercise one failed XPay webhook/signature, provider timeout or retry, and a
