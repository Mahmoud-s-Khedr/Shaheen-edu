...
| P1       | Subscriber export and platform reporting  | Admin-only aggregate reports cover revenue/discounts, refunds, payment attempts, registrations, active purchasers, and entitlement lifecycle. Approved non-PII CSVs are generated by an audited private export worker with a 24-hour artifact lifetime and 15-minute protected download URL. | Deploy the privacy/export migration, enable the rollout control only after Phase 0 finance sign-off, and run the staging queue/reconciliation drill. |
