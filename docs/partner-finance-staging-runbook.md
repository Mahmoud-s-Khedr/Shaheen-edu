...
Keep partner-ledger rollout allow-lists limited to the pilot publishers. Create one controlled manual-payment purchase and one XPay sandbox purchase. For each, retain the order ID, payment-attempt/provider reference, receipt reference, entitlement ID, allocation IDs, and any settlement or refund references.
